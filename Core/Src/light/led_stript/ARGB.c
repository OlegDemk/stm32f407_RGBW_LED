...
#include "light/led_stript/ARGB.h"  // include header file
#include "math.h"
...
extern DMA_HandleTypeDef (DMA_HANDLE_LEFT);  ///< DMA handler
extern DMA_HandleTypeDef (DMA_HANDLE_RIGHT);  ///< DMA handler
...
/// Static LED buffer
volatile u8_t RGB_BUF_LEFT[NUM_BYTES] = {0,};
volatile u8_t RGB_BUF_RIGHT[NUM_BYTES] = {0,};

/// Timer PWM value buffer
volatile dma_siz PWM_BUF_LEFT[PWM_BUF_LEN] = {0,};
volatile dma_siz PWM_BUF_RIGHT[PWM_BUF_LEN] = {0,};
/// PWM buffer iterator
volatile u16_t BUF_COUNTER_LEFT = 0;     // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Зробити для лівої і правої фари
volatile u16_t BUF_COUNTER_RIGHT = 0;

volatile u8_t ARGB_BR = 255;     ///< LED Global brightness
volatile ARGB_STATE ARGB_LOC_ST_LEFT; ///< Buffer send status
volatile ARGB_STATE ARGB_LOC_ST_RIGHT; ///< Buffer send status
 ...
 */
void ARGB_SetHSV(u16_t i, u8_t hue, u8_t sat, u8_t val) {   		 // DOESN"T USE IT <<<<<<<<<<
    uint8_t _r, _g, _b;                    // init buffer color
    HSV2RGB(hue, sat, val, &_r, &_g, &_b); // get RGB color
    ARGB_SetRGB_left(i, _r, _g, _b);     // set color
}
 ...
 * @param[in] val Value (brightness) [0..255]
 */
void ARGB_FillHSV_left(u8_t hue, u8_t sat, u8_t val) {				// DOESN"T USE IT <<<<<<<<<<
    uint8_t _r, _g, _b;                    // init buffer color
    HSV2RGB(hue, sat, val, &_r, &_g, &_b); // get color once (!)
    ARGB_FillRGB_left(_r, _g, _b);       // set color
}
 ...
 */
ARGB_STATE ARGB_Ready_left(void) {
    return ARGB_LOC_ST_LEFT;
}
// --------------------------------------------------------------
ARGB_STATE ARGB_Ready_right(void) {
    return ARGB_LOC_ST_RIGHT;
}
...
#if TIM_CH_RIGHT == TIM_CHANNEL_1
#define ARGB_TIM_DMA_ID TIM_DMA_ID_CC1
#define ARGB_TIM_DMA_CC TIM_DMA_CC1
#define ARGB_TIM_CCR CCR1
#elif TIM_CH_RIGHT == TIM_CHANNEL_2
#define ARGB_TIM_DMA_ID TIM_DMA_ID_CC2
#define ARGB_TIM_DMA_CC TIM_DMA_CC2
#define ARGB_TIM_CCR CCR2
#elif TIM_CH_RIGHT == TIM_CHANNEL_3
...
// Check channel
#if !(TIM_CH_LEFT == TIM_CHANNEL_1 || TIM_CH_LEFT == TIM_CHANNEL_2 || TIM_CH_LEFT == TIM_CHANNEL_3 || TIM_CH_LEFT == TIM_CHANNEL_4)
